...
namespace Api.Controllers.Api
{
    ...
    public class RolController : ApiController
    {
        ...
        private static Mittente GetMittente(SenderDto sender)
        {
            ...
            string businessName = (sender.businessName.Substring(sender.businessName.Length - 2) == "  " ? sender.businessName.Substring(0, sender.businessName.Length - 2) : sender.businessName);
            businessName = (businessName.Substring(businessName.Length - 1) == " " ? businessName.Substring(0, businessName.Length - 1) : businessName);

            Nominativo.RagioneSociale = businessName;

            Indirizzo.DUG = "";
            Indirizzo.NumeroCivico = "";
            Indirizzo.Toponimo = (sender.dug != "" ? sender.dug + " " : "") + sender.address + (sender.houseNumber != "" ? " " + sender.houseNumber : "");
        ...
        public static byte[] GetImgCover(string Pathfile)
        {
        ...
        public string RequestDCS(Guid guidUser, int id)
        {
            ...
            var dcs1 = service.RecuperaDocumentoFinale(name.codice);
            var nameFile = "";

            if (dcs1.CEResult.Type == "I")
            {
                var n = DateTime.Now.Ticks + ".pdf";
                nameFile = "/public/download/" + n;
                var path = HttpContext.Current.Server.MapPath(nameFile);
                var dc = GetDoc(dcs1.Documento.Contenuto, n);

                //System.IO.File.WriteAllBytes(path, dc);
        ...
        private static Documento[] GetDoc(byte[] bite,string n)
        {
            System.IO.FileInfo file;
            System.Security.Cryptography.MD5CryptoServiceProvider md5 = new System.Security.Cryptography.MD5CryptoServiceProvider();
